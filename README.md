...
## 楽天商品掲載の停止（2026-09-09）

商品カードと楽天データの読み込みを削除し、釣り方・レベル別の推奨スペックを残しています。復元用データはローカルの `archive/rakuten-2026-09-09/` に保存（Git対象外）。タスクスケジューラの「釣りタックルナビ 楽天商品自動確認」は無効化済みです。
